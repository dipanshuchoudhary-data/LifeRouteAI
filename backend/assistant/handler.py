"""
LifeRoute AI — Assistant Tool Handler
Executes assistant tools and orchestrates pipeline calls.
"""
...
async def _run_partial_pipeline(raw_input: str, location: dict | None, mode: str) -> dict:
    ...
    if mode == "list_only":
        ...
        return {"hospitals": get_all_hospitals() or MOCK_HOSPITALS}
...
async def invoke_tool(tool_name: str, parameters: dict) -> dict:
    """Execute a named assistant tool with parameters."""
    from assistant.tools import get_tool_by_name
...
def _slim_state(state: dict) -> dict:
    """Return a client-friendly subset of pipeline state."""
    return {
...
async def run_assistant_turn(
    ...
    *,
    ...
    """
    Process an assistant conversation turn.

    response_format:
      - "message" → message envelope with text, card, and structured data (default)
      - "json"    → flat JSON, convenient for lightweight chat widgets
      - "tool"    → tool invocation wrapper for function-calling clients
    """
